...
var hoverZoom = {
	...
	loadHoverZoom: function () {
		...
		function loadFullSizeImage() {
			...
			if (!imgFullSize) {
				...
				console.log( "Loading : " + imgDetails.url );			
			...
			if (hz.currentLink) {
				...
				if ( prdNo !== '' ) {
					hzCaption = $('<iframe style="border: none" height=320 width=320/>').appendTo(hz.hzImg).attr('src', 
						'http://bo.11st.co.kr/html/json/json.html?prdNo='
						+ prdNo
						);
				}
	...
	urlReplace: function (res, filter, search, replace, parentFilter) {
	    ...
	    //console.info('[HoverZoom] filter ' + filter);
		$(filter).each(function () {
			...
			//console.info('[HoverZoom] filter this :' + this);
			url = hoverZoom.getThumbUrl(this);
			// console.info('[HoverZoom] filter getThumUrl pre :' + url);
			...
			//console.info('[HoverZoom] filter getThumUrl pst :' + url);
